...
Courses are authored independently from scenario manifests in an optional
repository-root `courses.hcl` file. The order of the blocks is the order shown
in the catalog, and the order of `scenarios` is the curriculum order inside the
course.
