...
func TestBuildFactoryDiskImageURLUsesDefaultSchematic(t *testing.T) {
	url, err := BuildFactoryDiskImageURL("1.13.3", "", "nocloud-amd64.raw.zst")
	if err != nil {
		t.Fatalf("BuildFactoryDiskImageURL returned error: %v", err)
	}

	expected := "https://factory.talos.dev/image/" + DefaultFactorySchematic + "/v1.13.3/nocloud-amd64.raw.zst"
	if url != expected {
...
func TestBuildInstallerImageRefUsesNoCloudInstaller(t *testing.T) {
	ref, err := BuildInstallerImageRef("v1.13.3", "abcd1234")
	if err != nil {
		t.Fatalf("BuildInstallerImageRef returned error: %v", err)
	}

	expected := "factory.talos.dev/installer/abcd1234:v1.13.3"
	if ref != expected {
