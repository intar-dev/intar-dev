...
test("an enrollment can only bind one credential and cannot revive revoked access", async () => {
  ...
  try {
    sqlite.exec("CREATE TABLE user (id TEXT, role TEXT, banned INTEGER, deleted_at INTEGER); CREATE TABLE member (user_id TEXT, organization_id TEXT, role TEXT); CREATE TABLE access_allowlist (user_id TEXT, state TEXT, source_invite_id TEXT, source_lease_id TEXT, granted_at INTEGER); CREATE TABLE host_enrollments (token_hash TEXT PRIMARY KEY, host_id TEXT, user_id TEXT, name TEXT, scope TEXT, role TEXT, organization_id TEXT, source_invite_id TEXT, source_lease_id TEXT, granted_at INTEGER, expires_at INTEGER, claimed_at INTEGER, credential_hash TEXT, revoked_at INTEGER); CREATE TABLE agent_hosts (id TEXT PRIMARY KEY, user_id TEXT, name TEXT, scope TEXT, role TEXT, organization_id TEXT, credential_generation INTEGER, scenario_enabled INTEGER, disabled INTEGER, connected INTEGER, created_at INTEGER, updated_at INTEGER); CREATE TABLE agent_bootstrap_tokens (id TEXT PRIMARY KEY, host_id TEXT, token_hash TEXT, credential_generation INTEGER, created_at INTEGER, revoked_at INTEGER, expires_at INTEGER); INSERT INTO user VALUES ('alice', 'user', 0, NULL); INSERT INTO access_allowlist VALUES ('alice', 'active', 'invite', 'lease', 1);");
    ...
    sqlite.prepare("INSERT INTO host_enrollments VALUES (?, 'host-a', 'alice', 'Server', 'personal', 'agent', NULL, 'invite', 'lease', 1, ?, NULL, NULL, NULL)")
