...
function actionWrite(
  ...
  return canonicalWrite(
    request,
    {
      ...
      actionIds: [action.id],
      state: action.status,
      ...(action.error ? { errorCode: action.error.code } : {}),
    },
    options,
  );
}
...
export async function runOperation(
  ...
  switch (operation.kind) {
    ...
    case "preflight_capacity": {
      if (!Number.isSafeInteger(operation.requestedSeats) || operation.requestedSeats < 0) {
        throw new ProviderServiceError({
          code: "invalid_provider_request",
          message: "Hetzner capacity preflight requirements are invalid",
          retryable: false,
        });
      }
      const catalog = await client.observeCatalog({
        requiredServerTypes: [operation.serverType],
        permittedLocations: operation.permittedLocations,
        systemImage: operation.systemImage,
      });
      const serverType = catalog.serverTypes.find(
        (candidate) => candidate.name === operation.serverType,
      );
      const locations = serverType?.locations ?? [];
      const availableLocations = operation.permittedLocations.filter(
        (location) =>
          locations.some(
            (candidate) =>
              candidate.name === location &&
              candidate.available === true &&
              candidate.deprecation == null,
          ),
      );
      const preferredLocation = availableLocations[0] ?? null;
      // Hetzner exposes a boolean location/type availability signal but no
      // project quota count. Preserve that weaker basis explicitly while the
      // generic control plane supplies the numeric ceiling from the configured
      // connection maximum and its current allocation ledger.
      const availableSeats =
        preferredLocation === null ? 0 : operation.requestedSeats;
      const reasons =
        operation.requestedSeats === 0
          ? []
          : preferredLocation === null
            ? ["Hetzner reports no available approved location for the exact server type"]
            : [
                "Hetzner reports the exact type available but exposes no numeric project quota; capacity is not guaranteed until allocation",
              ];
      const data: ProviderCapacityObservation = {
        observedAt: observedAt(options),
        requestedSeats: operation.requestedSeats,
        availableSeats: Math.min(operation.requestedSeats, availableSeats),
        preferredLocation,
        availableLocations,
        capacityBasis:
          preferredLocation === null ? "unavailable" : "availability_only",
        reasons,
      };
      return {
        data,
