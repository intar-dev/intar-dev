...
import { SerializedOperationQueue } from "@intar/provider-worker-core";
import { HETZNER_PROVIDER_CAPABILITIES } from "./capabilities";
import type {
...
export { HETZNER_PROVIDER_CAPABILITIES } from "./capabilities";

export class HetznerConnectionDO extends DurableObject<Env> {
  ...
  async connectProject(
    ...
    return this.#execute(() =>
      connectProjectWithProvider(request, this.env.HETZNER_PROVIDER_CREDENTIAL_KEK_V1),
    );
  }
  ...
  async rotateCredential(
    ...
    return this.#execute(() =>
      rotateCredentialWithProvider(request, this.env.HETZNER_PROVIDER_CREDENTIAL_KEK_V1),
    );
  }
  ...
  async runOperation(
    ...
    return this.#execute(() =>
      runProviderOperation(request, this.env.HETZNER_PROVIDER_CREDENTIAL_KEK_V1),
    );
  }
  ...
  async reconcile(request: RunOperationRequest): Promise<ProviderRpcResult<ReconcileResult>> {
    return this.#execute(async () => {
      ...
      const result = await runProviderOperation(
        request,
        this.env.HETZNER_PROVIDER_CREDENTIAL_KEK_V1,
      );
...
export class HetznerProviderService extends WorkerEntrypoint<Env> {
  capabilities(): ProviderCapabilities<"hetzner_cloud"> {
    return HETZNER_PROVIDER_CAPABILITIES;
  }

  #connection(connectionId: string): DurableObjectStub<HetznerConnectionDO> {
    if (!/^[A-Za-z0-9._-]{8,128}$/u.test(connectionId)) throw new TypeError("Invalid connection id");
    return this.env.HETZNER_CONNECTIONS.getByName(`hetzner:${connectionId}`);
  }

  async #call<T>(
    connectionId: string,
    call: (
      connection: DurableObjectStub<HetznerConnectionDO>,
    ) => PromiseLike<ProviderRpcResult<T>>,
  ): Promise<ProviderRpcResult<T>> {
    let connection: DurableObjectStub<HetznerConnectionDO>;
    try {
...
export default HetznerProviderService;
