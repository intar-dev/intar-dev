...
import type { RuntimeProviderSelection } from "@intar/workshop-contracts";
...
export async function createWorkshopSession(params: {
  ...
  await appendWorkshopEvent(db, {
    ...
    payload: {
      templateRevisionId: params.templateRevisionId,
      runtimeProfileId: preparedProvider.runtimeProfileId,
      runtimeProfile: preparedProvider.profileId,
      runtimeProvider: preparedProvider.providerKind,
      providerConnectionId: preparedProvider.connectionId,
      machineType: preparedProvider.resolvedProfile.machineType,
    },
    createdAt: now,
  });
