...
import {
  isWorkshopMulticloudRuntimeEnabledForOrganization,
  requireWorkshopMulticloudRuntimeEnabledForOrganization,
  WORKSHOP_MULTICLOUD_RUNTIME_FEATURE_FLAG,
...
describe("multi-cloud workshop runtime feature flag", () => {
  it("uses organization targeting and defaults off", async () => {
    const service = {
      getBoolean: vi.fn().mockResolvedValue(false),
    };
    await expect(
      isWorkshopMulticloudRuntimeEnabledForOrganization("org-a", service),
    ).resolves.toBe(false);
    ...
    await expect(
      requireWorkshopMulticloudRuntimeEnabledForOrganization("org-a", service),
