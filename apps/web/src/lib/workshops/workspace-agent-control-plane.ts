...
export async function issueWorkspaceAgentBootstrap(
  ...
  const source = await env.DB.prepare(
    `SELECT
       ...
       bundle.id AS checkpoint_bundle_id,
     ...
     INNER JOIN runtime_checkpoint_bundles bundle
       ON bundle.checkpoint_id = execution.checkpoint_id
     WHERE execution.id = ?
         ...
         (
           ...
           AND EXISTS (
             ...
             JOIN workshop_runtime_profile_certifications certification
               ON certification.runtime_profile_id = selection.runtime_profile_id
              AND certification.state = 'verified'
              AND certification.deletion_confirmed_at IS NOT NULL
  ...
  const result = await env.DB.prepare(
    `INSERT INTO runtime_guest_credentials (
       ...
       checkpoint_bundle_id, created_at, updated_at
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(execution_id) DO UPDATE SET
       ...
       checkpoint_bundle_id = excluded.checkpoint_bundle_id,
       updated_at = excluded.updated_at
     WHERE runtime_guest_credentials.bootstrap_consumed_at IS NULL`,
  )
    .bind(
      id,
      source.execution_id,
...
# Set-ID packages required by the harness must be installed before the
# restricted workspace-agent service starts.
packages:
  - ca-certificates
  - curl
  - openssh-server
...
export function requiredWorkspaceAgentControlPlaneBaseUrl(): string {
  const value = (
    env as Cloudflare.Env & { WORKSPACE_AGENT_CONTROL_PLANE_URL?: string }
...
export async function resolveWorkspaceGuestTools(
  ...
  if (
    !/^[a-f0-9]{64}$/u.test(pinned.workspaceAgentSha256) ||
    !/^[a-f0-9]{64}$/u.test(pinned.kinoSha256)
  ) {
    throw appError(
      503,
  ...
  const agentKey = `workspace-agent/releases/${pinned.workspaceAgentSha256}/intar-workspace-agent`;
  const kinoKey = `workspace-agent/kino/releases/${pinned.kinoSha256}/kino`;
  const [agent, kino] = await Promise.all([
    env.VM_IMAGE_REGISTRY_BUCKET.head(agentKey),
    env.VM_IMAGE_REGISTRY_BUCKET.head(kinoKey),
  ]);
  if (
    !agent ||
    agent.size <= 0 ||
    ...
    throw appError(
      503,
      "workspace_agent_binary_unavailable",
      "the pinned workspace guest binaries are unavailable",
    );
  }
  return {
    agent: {
      sha256: pinned.workspaceAgentSha256,
      sizeBytes: agent.size,
      url: new URL(
        `/api/runtime/workspace-agent/binaries/${pinned.workspaceAgentSha256}`,
        controlPlaneBaseUrl,
      ).toString(),
    },
    kino: {
      sha256: pinned.kinoSha256,
      sizeBytes: kino.size,
      url: new URL(
        `/api/runtime/workspace-agent/kino/binaries/${pinned.kinoSha256}`,
        controlPlaneBaseUrl,
      ).toString(),
    },
  };
