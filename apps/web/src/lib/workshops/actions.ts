...
async function provisionWorkshopRequestOrDeferReplacement(
  ...
  try {
    ...
    if (
      ...
      error.code === "workshop_restore_cleanup_pending"
    ) {
      // The queued generation and checkpoint-restore event are durable. The
      // minute sweep resumes provisioning only after the predecessor provider
      // allocation has confirmed complete deletion.
      return;
    }
    throw error;
  }
