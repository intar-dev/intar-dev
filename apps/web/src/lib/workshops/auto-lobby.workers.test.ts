...
async function createSession(lobbyOpensAt: number): Promise<string> {
  ...
  await drizzle(env.DB).batch([
    drizzle(env.DB).insert(workshopRuntimeProfiles).values({
      id: profileId,
      templateRevisionId: template.revision.id,
      profileId: "agent-x86",
      providerKind: "agent_kvm",
      vmId: "workspace",
      systemImage: "scheduled-workshop",
      architecture: "x86_64",
      cpuMillis: 1_000,
      memoryMib: 1_024,
      diskMib: 10_240,
    ...
    drizzle(env.DB).insert(workshopRuntimeProfileCertifications).values({
      id: `cert-${template.revision.id}`,
      ...
      runtimeProfiles: [
        {
          ...
          provider: "agent_kvm",
          vmId: "workspace",
          requestedSystemImage: "scheduled-workshop",
          immutableSystemImage: "scheduled-workshop",
          locations: [],
          hardware: {
            architecture: "x86_64",
            cpuMillis: 1_000,
