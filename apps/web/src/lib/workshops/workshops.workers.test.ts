...
async function createTemplateFixture(manifest = workshopManifest()) {
  ...
  const profile = manifest.workspace.runtimeProfiles[0];
...
function workshopManifest(
  ...
  return {
    ...
    workshop: {
      ...
      attribution: {
        title: "Platform Engineering Workshop",
        url: "https://example.test/platform-engineering",
        license: "Apache-2.0",
      },
      ...
      runtimeProfiles: [
        {
          ...
          provider: "agent_kvm",
          vmId: "workshop",
          requestedSystemImage: "workshop-test",
          immutableSystemImage: "workshop-test",
          locations: [],
          hardware: {
            architecture: "x86_64",
            cpuMillis: 4_000,
...
): WorkshopManifestV2["workspace"]["checkpoints"][number] {
  return {
    id,
    label: id,
    vmImages: [
...
function stretchModule(
  id: string,
  checkpointId: string,
): WorkshopManifestV2["modules"][number] {
  return {
    id,
    title: id,
    tier: "stretch",
...
function stretchAgenda(moduleId: string): WorkshopManifestV2["agenda"][number] {
  return {
    id: `${moduleId}-agenda`,
    kind: "tinker",
    title: moduleId,
...
function stretchSlide(
  moduleId: string,
): WorkshopManifestV2["presentation"]["slides"][number] {
  return {
    id: `${moduleId}-slide`,
    layout: "content",
    title: moduleId,
