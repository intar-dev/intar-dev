...
import {
  ...
  archiveProviderWorkshopRuntime,
  requireProductionRuntimeProviderAdapter,
...
export async function provisionWorkshopRequest(
  ...
  // The same fail-closed registry governs agent KVM and both direct-cloud
  // backends. A missing adapter is never inferred or substituted.
  requireProductionRuntimeProviderAdapter(providerKind);
  if (providerKind !== "agent_kvm") {
    return allocateProviderWorkshopRuntime(request, {
...
export async function teardownWorkshopSessionRuntimes(input: {
  ...
  const rows = await env.DB.prepare(
    ...
    .all<{
      ...
      provider_kind: RuntimeExecutionHandle["providerKind"] | null;
    }>();
  ...
  for (const row of rows.results) {
    ...
    if (row.runtime_execution_id && row.ordinal) {
      if (row.provider_kind && row.provider_kind !== "agent_kvm") {
  ...
  if (providerCleanupPending) {
    throw appError(
      ...
      "one or more direct-cloud learner resources are still being deleted",
    );
  }
}
...
/**
 * Reconstruct direct-cloud learners that remained unhealthy for three minutes
 * after the harness-issued reboot. The pending generation is durable, so a
 * provider deletion that spans multiple sweeps is retried without allocating
 * a second replacement or losing Workshop progress.
 */
export async function recoverWorkshopRuntimesFromFailedProvider(input: {
  now?: number;
  maxWorkspaces?: number;
} = {}): Promise<WorkshopProvisioningOutcome[]> {
  const now = input.now ?? Date.now();
  const maxWorkspaces = Math.max(
    1,
    Math.min(MAX_FAILED_HOST_RECOVERY_BATCH_SIZE, input.maxWorkspaces ?? 8),
  );
  const rows = await loadDeferredWorkshopRuntimeReplacementCandidates(
    maxWorkspaces,
  );
  const outcomes: WorkshopProvisioningOutcome[] = [];
  for (const row of rows) {
    let generationId = row.generation_id;
    try {
      const context = await loadWorkshopManifestForSession(row.session_id);
      const reusableGeneration = row.runtime_execution_id === null;
      const checkpointId = reusableGeneration
      ...
      generationId = reusableGeneration ? row.generation_id : createAppId();
      const ordinal = reusableGeneration ? row.ordinal : row.ordinal + 1;
      const message =
        row.request_event_type === "workspace.checkpoint_restore_requested"
          ? `Restoring ${checkpointId}; work after that checkpoint will be lost.`
          : `Provider health recovery is reconstructing ${checkpointId}; work since that checkpoint may be lost.`;
      const db = workshopDb();
      if (reusableGeneration) {
        await db
          .update(workshopWorkspaceGenerations)
        ...
        await db.batch([
          ...
          db
            .insert(workshopEvents)
            .values({
              id: workshopGenerationEventId("provider-recovery", generationId),
              organizationId: row.organization_id,
              sessionId: row.session_id,
              actorUserId: null,
              type: "workspace.provider_failure_recovery_requested",
              payloadJson: {
                workspaceId: row.workspace_id,
                failedExecutionId: row.failed_execution_id,
                checkpointId,
                generationId,
                lostWorkWarning: message,
              },
      ...
      const execution = await provisionWorkshopRequest(
        {
          organizationId: row.organization_id,
          sessionId: row.session_id,
          templateRevisionId: row.template_revision_id,
...
export interface DeferredWorkshopRuntimeReplacementCandidate {
  workspace_id: string;
  session_id: string;
  user_id: string;
  terminal_route_usernames_json: string | string[];
  ...
  organization_id: string;
  template_revision_id: string;
  failed_execution_id: string | null;
  request_event_type:
    | "workspace.provider_failure_recovery_requested"
    | "workspace.checkpoint_restore_requested"
    | null;
}

export async function loadDeferredWorkshopRuntimeReplacementCandidates(
  limit = 8,
): Promise<DeferredWorkshopRuntimeReplacementCandidate[]> {
  const rows = await env.DB.prepare(
    `SELECT
       workspace.id AS workspace_id,
       workspace.session_id,
       ...
       session.organization_id,
       session.template_revision_id,
       failed_execution.id AS failed_execution_id,
       (
         SELECT event.type
         FROM workshop_events event
         WHERE event.session_id = workspace.session_id
         ...
         LIMIT 1
       ) AS request_event_type
     FROM workshop_workspaces workspace
     JOIN workshop_workspace_generations generation
       ON generation.id = workspace.current_generation_id
     JOIN workshop_sessions session ON session.id = workspace.session_id
     LEFT JOIN runtime_executions failed_execution
       ON failed_execution.id = generation.runtime_execution_id
      AND failed_execution.domain_kind = 'workshop'
     LEFT JOIN runtime_provider_allocations allocation
       ON allocation.execution_id = failed_execution.id
     WHERE session.state IN ('lobby', 'live')
       AND (
         (
           allocation.state IN ('failed', 'deleted')
           AND allocation.last_error_code = 'reconstruct_required'
         )
         OR (
           workspace.state IN ('recovering', 'failed')
           ...
           AND generation.state IN ('queued', 'failed')
           AND EXISTS (
             SELECT 1 FROM workshop_events event
             WHERE event.session_id = workspace.session_id
               AND event.type IN (
                 'workspace.provider_failure_recovery_requested',
                 'workspace.checkpoint_restore_requested'
    ...
    .bind(Math.max(1, Math.min(MAX_FAILED_HOST_RECOVERY_BATCH_SIZE, limit)))
    .all<DeferredWorkshopRuntimeReplacementCandidate>();
  return rows.results;
}
