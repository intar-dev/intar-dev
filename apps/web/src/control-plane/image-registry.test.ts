...
describe("course catalog bundle metadata", () => {
  ...
  it("normalizes optional HTTPS course credits", async () => {
    const result = await readBundleMeta(
      JSON.stringify(
        sourceMeta({
          course_catalog: {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.bundleMeta.courseCatalog?.courses[0]?.credits).toEqual([
  ...
  it("rejects invalid versions, shapes, IDs, and duplicate membership", async () => {
    const invalidSnapshots: unknown[] = [
      ...
      },
      {
        version: 1,
        mode: "replace",
        courses: [courseMeta({ credits: "not-an-array" })],
      },
      {
        version: 1,
        mode: "replace",
      ...
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [
              { label: "Rawkode\nAcademy", url: "https://rawkode.academy" },
            ],
          }),
        ],
      },
      ...
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [
              { label: "Rawkode Academy", url: "http://rawkode.academy" },
            ],
          }),
        ],
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [{ label: "Rawkode Academy", url: "https://" }],
          }),
        ],
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [
              {
                label: "Rawkode Academy",
                url: "https://learner:secret@rawkode.academy",
              },
            ],
          }),
        ],
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [
              { label: "Rawkode Academy", url: "https://rawkode.academy" },
              { label: "Rawkode Academy", url: "https://example.test" },
            ],
          }),
        ],
      },
      {
        ...
        courses: [
          courseMeta({
            credits: [
              { label: "Rawkode Academy", url: "https://rawkode.academy" },
              { label: "Rawkode", url: "https://RAWKODE.academy" },
            ],
          }),
        ],
      },
