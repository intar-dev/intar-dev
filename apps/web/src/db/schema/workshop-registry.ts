...
export type WorkshopPublicationStatus =
  ...
  | "failed"
  | "published";
export type WorkshopCertificationState =
  | "verifying"
  | "verified"
  | "failed"
  | "cleanup_pending";
...
export const workshopPublications = sqliteTable(
  "workshop_publications",
  {
    ...
    runtimeProfileResolutionsJson: jsonText<Array<Record<string, unknown>>>(
    ...
    certificationState: text(
      "certification_state",
    ).$type<WorkshopCertificationState | null>(),
  ...
  (table) => [
    ...
    check(
      "workshop_publications_certification_state_valid",
      sql`${table.certificationState} is null OR ${table.certificationState} in ('verifying', 'verified', 'failed', 'cleanup_pending')`,
    ),
    check(
      "workshop_publications_runtime_profiles_json_valid",
      sql`json_valid(${table.runtimeProfileResolutionsJson})`,
    ),
  ],
);
