...
runs inside your Intar workspace, readable end to end.
...
2. **The moment.** Two terminals:
   ...
   - open the released **Cloudbox Console** app, go to **Gallery**, and upload any
     JPEG/PNG.
...
5. **The flourish.** Observability is an on-demand capability — enable the Victoria stack +
   ...
   in the released **Grafana** app → Explore → **VictoriaTraces** and see the chain —
   portal → uploader → broker → resizer — as one waterfall. Hint 5 if the Jaeger
   trace view is new to you.
6. Run `./verify.sh`.
...
- **Sepia.** Fork `apps/resizer` into a sepia-filter service writing `sepia/`,
  ...
  from parts you own.
- You built S3-events → queue → function in one workspace. Sketch which managed products
