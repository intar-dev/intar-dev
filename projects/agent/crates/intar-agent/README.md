...
Example config: `projects/agent/config.example.toml`
...
[vm_defaults.network]
bridge = "br0"
guest_cidr = "10.200.0.0/24"
gateway = "10.200.0.1"
dns = ["1.1.1.1", "8.8.8.8"]

[image_registry]
url = "https://u559333.your-storagebox.de"
refresh_interval_minutes = 15
# Optional HTTP basic auth used for the registry index, qcow2 image downloads, and sha256 sidecars.
username = "u559333"
password = ""
...
The image registry is expected to expose:

- `<url>/IMAGE_KEY.qcow2`
- `<url>/IMAGE_KEY.qcow2.sha256`

`intar-agent` polls the registry every `refresh_interval_minutes`, caches any newly discovered `.qcow2` images, and uses the sidecar `.sha256` file for verification. Verification mismatches are logged as warnings and do not block image use.
