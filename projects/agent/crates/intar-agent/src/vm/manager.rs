...
use crate::kino_probe::{
    ProbeCollectionState, ProbeSnapshotView, ProbeUpdateEnvelope, build_probe_snapshot_view,
...
pub struct CreateScenarioVmRuntime {
    pub stargate_target_public_key_openssh: String,
...
#[derive(Debug, Clone)]
enum LaunchConfig {
    ScenarioRuntime { runtime: CreateScenarioVmRuntime },
}
...
impl LaunchConfig {
    fn config_disk_filename(&self) -> &'static str {
        "runtime.img"
    }

    fn config_disk_id_suffix(&self) -> &'static str {
        "runtime"
    }
}

const LEASE_EXPIRY_ERROR_LOG_INTERVAL_S: i64 = 60;
const NFT_VM_NET_TABLE: &str = "intar_agent_vm_net";
const NFT_VM_NET_POSTROUTING_CHAIN: &str = "postrouting";
const NFT_VM_NET_PREROUTING_CHAIN: &str = "prerouting";
...
impl VmManager {
    ...
    pub async fn create_scenario_vm(
        ...
        let stargate_target_public_key_openssh = req
            .runtime
            .stargate_target_public_key_openssh
            .trim()
            .to_string();
        if stargate_target_public_key_openssh.is_empty() {
            return Err(ApiError::bad_request(
                "runtime.stargate_target_public_key_openssh must not be empty",
            ));
        }
        ...
        self.queue_vm_create(QueueVmCreateRequest {
            ...
            launch_config: LaunchConfig::ScenarioRuntime { runtime },
        })
        .await
    }
    ...
    async fn queue_vm_create(
        ...
        let scenario_runtime = match &launch_config {
            LaunchConfig::ScenarioRuntime { runtime } => runtime,
        };
        let guest_ip_cidr = match scenario_runtime.network.as_ref() {
            Some(network) => network.guest_ip_cidr.clone(),
            None => allocate_guest_ip_cidr(&self.inner).await?,
        };
        let mut network = CreateVmNetwork {
            guest_ip_cidr,
            gateway: scenario_runtime
                .network
                .as_ref()
                .and_then(|network| network.gateway.clone())
                .unwrap_or_else(|| self.inner.defaults.network.gateway.clone()),
            dns: scenario_runtime
                .network
                .as_ref()
                .and_then(|network| network.dns.clone())
        ...
        ensure_vm_network_reconciled(&self.inner)
            .await
            .map_err(|e| {
                ApiError::internal(format!("failed to reconcile vm host networking: {e}"))
            })?;
        ...
        let config_disk_path = vm_dir.join(launch_config.config_disk_filename());
        ...
        let launch_config_for_task = launch_config.clone();
        ...
        tokio::spawn(async move {
            ...
            let create_input = RunCreateInput {
                ...
                boot: boot_for_task,
                launch_config: &launch_config_for_task,
                tap: &tap_for_task,
    ...
    pub async fn list_inventory_vms(&self) -> Result<Vec<Value>> {
        let states = {
            let states = self.inner.states.read().await;
            states.values().cloned().collect::<Vec<_>>()
        };
        stream::iter(states)
            .then(|vm| async move { inventory_value_from_status(&vm).await })
            .try_collect()
            .await
    }

    pub async fn list_terminal_states(&self) -> Vec<VmTerminalState> {
        let ssh_access = self.inner.ssh_access.clone();
        let states = {
            let states = self.inner.states.read().await;
            states.values().cloned().collect::<Vec<_>>()
        };
        stream::iter(states)
            .then(|vm| {
                let ssh_access = ssh_access.clone();
                async move { build_terminal_state_from_status(&vm, &ssh_access).await }
            })
            .filter_map(|state| async move { state.ok().flatten() })
            .collect()
            .await
    }
    ...
    pub async fn get_terminal_state(
        ...
        run_id: &str,
        vm_name: &str,
    ) -> Result<VmTerminalState, ApiError> {
        let vm = {
            let states = self.inner.states.read().await;
        ...
        let state_run_id = vm
            ...
            .ok_or_else(|| ApiError::not_found("vm run id not found"))?;
        if state_run_id != run_id {
            return Err(ApiError::not_found("vm run id mismatch"));
        }

        build_terminal_state_from_status(&vm, &self.inner.ssh_access)
            .await
            .map_err(|error| ApiError::internal(error.to_string()))?
            .ok_or_else(|| ApiError::not_found("terminal state unavailable"))
    }
...
async fn run_create(inner: &Arc<Inner>, req: RunCreateInput<'_>) -> Result<()> {
    ...
    let base_path = image_cache::ensure_cached(
        req.image_key,
    ...
    .await
    .context("failed to ensure base image is cached")?;
    ensure_create_not_deleted(inner, req.name).await?;

    set_state(inner, req.name, VmLifecycleState::PreparingDisks).await;

    let base_format = qemu_img::detect_format(req.qemu_img, &base_path)
        .await
        .with_context(|| {
            format!(
                "failed to detect base image format at {}",
                base_path.display()
            )
        })?;
    ...
    qemu_img::convert_to_raw(req.qemu_img, &base_path, &base_format, req.root_disk_path)
        .await
        .context("failed to create root disk (qemu-img convert -O raw)")?;
    ...
    let runtime = match req.launch_config {
        LaunchConfig::ScenarioRuntime { runtime } => runtime,
    };
    info!(
    ...
    let stargate_target_public_key_openssh = runtime.stargate_target_public_key_openssh.clone();
    ...
    tokio::task::spawn_blocking(move || {
        runtime_disk::write_runtime_disk(&runtime_disk::RuntimeDiskInput {
            ...
            stargate_target_public_key_openssh: &stargate_target_public_key_openssh,
            kino_vsock_cid,
            kino_vsock_port,
            hostname: &hostname,
            network: &network,
    ...
    ensure_tap_ready(&inner.defaults.network.bridge, req.tap).await?;
    ensure_create_not_deleted(inner, req.name).await?;
        ...
        disks: Some(vec![
            ...
            DiskConfig {
                ...
                id: Some(format!(
                    "{}-{}",
                    req.name,
                    req.launch_config.config_disk_id_suffix()
                )),
                image_type: Some(DiskImageType::Raw),
            },
            DiskConfig {
...
async fn wait_for_scenario_runtime_ready(
    ...
    loop {
        ...
        let probe_error = match fetch_probe_snapshot(&kino_vsock_path, kino_vsock_port).await {
            Ok(result) => {
                return Ok(ProbeUpdateEnvelope::from_poll_result(
                    vm_name, &run_id, result,
                ));
            }
            Err(error) => {
                debug!(
                    vm = vm_name,
                    error = %error,
                    "scenario runtime probe snapshot not ready yet"
                );
                error
            }
        };
        ...
        if Instant::now() >= deadline {
            stop_booting_vm(inner, vm_name).await;
            return Err(probe_error).context(scenario_runtime_timeout_context(
                &kino_vsock_path,
                saw_kino_vsock_socket,
            ));
        }

        tokio::time::sleep(Duration::from_millis(SCENARIO_READY_POLL_INTERVAL_MILLIS)).await;
    }
}
...
async fn upload_vm_run_artifacts(
    ...
    let begin_payload = begin_response
        .json::<RunUploadBeginResponse>()
        .await
        .context("failed to decode run begin response")?;

    let prepared = prepared.clone();
    let vm_name = vm_name.to_string();
    let lease_token = begin_payload.lease_token.clone();
    stream::iter(artifacts.into_iter().map(|artifact| {
        ...
        async move {
            upload_single_artifact(inner, &prepared, &artifact, &access_token, &lease_token)
                .await
    ...
    let complete_url = format!(
        "{}/agent/runs/{}/complete",
        inner.bridge.base_url, prepared.run_id
    );
    let response = inner
        .http
        .post(&complete_url)
        .bearer_auth(&access_token)
        .header("x-run-upload-lease", &begin_payload.lease_token)
        .send()
        .await
...
async fn upload_single_artifact(
    ...
    let begin_url = format!(
        "{}/agent/runs/{}/artifacts/{}/multipart-begin",
        inner.bridge.base_url, prepared.run_id, artifact.ordinal
    );
    let begin_response = inner
        .http
        .post(&begin_url)
        .bearer_auth(access_token)
        .header("x-run-upload-lease", lease_token)
        .send()
        .await
    ...
    loop {
        ...
        let part_url = format!(
            "{}/agent/runs/{}/artifacts/{}/parts/{}",
            inner.bridge.base_url, prepared.run_id, artifact.ordinal, part_number
        );
        let response = inner
            .http
            .put(&part_url)
...
async fn run_probe_worker_task(
    ...
    let (mut last_fingerprint, mut last_snapshot) =
    ...
    loop {
        ...
        let update = match fetch_probe_snapshot(&kino_vsock_path, kino_vsock_port).await {
            Ok(result) => {
                last_snapshot = Some(build_probe_snapshot_view(&result));
                ProbeUpdateEnvelope::from_poll_result(&vm_name, &run_id, result)
            }
            Err(error) => ProbeUpdateEnvelope::from_error_with_snapshot(
                &vm_name,
                &run_id,
                error.to_string(),
                last_snapshot.clone(),
        ...
        if last_fingerprint
            .as_deref()
            .is_some_and(|fingerprint| fingerprint == update.fingerprint)
        {
            continue;
        }

        last_fingerprint = Some(update.fingerprint.clone());
        last_snapshot = Some(update.snapshot_view());
...
async fn ensure_vm_network_reconciled(inner: &Inner) -> Result<()> {
    let guest_cidr = inner.defaults.network.guest_cidr.trim();
    if guest_cidr.is_empty() {
        anyhow::bail!("vm_defaults.network.guest_cidr is empty");
    }
    let (guest_ip_raw, prefix_raw) = guest_cidr
        .split_once('/')
        .ok_or_else(|| anyhow::anyhow!("vm_defaults.network.guest_cidr must be CIDR"))?;
    let _: Ipv4Addr = guest_ip_raw
        .parse()
        .map_err(|_| anyhow::anyhow!("vm_defaults.network.guest_cidr must be IPv4 CIDR"))?;
    let prefix: u8 = prefix_raw
        .parse()
        .map_err(|_| anyhow::anyhow!("vm_defaults.network.guest_cidr has invalid prefix"))?;

    let bridge = inner.defaults.network.bridge.trim();
    if bridge.is_empty() {
        anyhow::bail!("vm_defaults.network.bridge must not be empty");
    }
    let gateway_raw = inner.defaults.network.gateway.trim();
    if gateway_raw.is_empty() {
        anyhow::bail!("vm_defaults.network.gateway must not be empty");
    }
    let gateway: Ipv4Addr = gateway_raw
        .parse()
        .map_err(|_| anyhow::anyhow!("vm_defaults.network.gateway must be an IPv4 address"))?;

    ensure_bridge_ready(bridge, gateway, prefix).await?;

    ensure_ipv4_forwarding_enabled().await?;
    ensure_nft_vm_nat_base().await?;
    ...
    let ssh_forwards = {
        ...
        states
            ...
            .filter_map(|vm| {
                let details = vm.details.as_ref()?;
                let guest_ip = details.guest_ip.as_deref()?.trim();
                let public_port = details.ssh_public_port?;
                if guest_ip.is_empty() {
                    return None;
                }
                Some((vm.name.clone(), public_port, guest_ip.to_string()))
            })
            .collect::<Vec<_>>()
    };
    ...
    run_command(
        ...
        &[
            ...
            "ip",
            NFT_VM_NET_TABLE,
            NFT_VM_NET_POSTROUTING_CHAIN,
        ],
        "flush vm egress nat chain",
    )
    .await?;
    run_command(
        "nft",
        &[
            ...
            "ip",
            NFT_VM_NET_TABLE,
            NFT_VM_NET_PREROUTING_CHAIN,
        ],
        "flush vm ssh forwarding chain",
    )
    .await?;
    run_command(
        "nft",
        ...
        "add vm egress nat rule",
    )
    .await?;
    let ssh_forward_count = ssh_forwards.len();
    if inner.ssh_access.enabled {
        for (vm_name, public_port, guest_ip) in &ssh_forwards {
            let public_port_string = public_port.to_string();
            let target = format!("{guest_ip}:22");
            run_command(
                "nft",
                &[
                    "add",
                ...
                "add vm ssh forwarding rule",
            )
            .await
            .with_context(|| {
                format!(
                    "failed to reconcile ssh forwarding for vm {vm_name} on port {public_port_string}"
                )
            })?;
        }
    }
...
async fn ensure_nft_vm_nat_base() -> Result<()> {
    let table_exists = Command::new("nft")
        .arg("list")
        .arg("table")
        .arg("ip")
        .arg(NFT_VM_NET_TABLE)
        .output()
        .await
    ...
    let chain_exists = Command::new("nft")
        ...
        .arg("ip")
        .arg(NFT_VM_NET_TABLE)
        .arg(NFT_VM_NET_POSTROUTING_CHAIN)
        .output()
        .await
    ...
    if !chain_exists {
        run_command(
            ...
            "create vm nat postrouting chain",
        )
        .await?;
    }

    let prerouting_chain_exists = Command::new("nft")
        .arg("list")
        .arg("chain")
        .arg("ip")
        .arg(NFT_VM_NET_TABLE)
        .arg(NFT_VM_NET_PREROUTING_CHAIN)
        .output()
        .await
        .context("failed to execute nft list prerouting chain")?
        .status
        .success();
    ...
    if !prerouting_chain_exists {
        run_command(
            ...
            "create vm nat prerouting chain",
        )
        .await?;
    }
...
async fn inventory_value_from_status(vm: &VmStatusResponse) -> Result<Value> {
    serde_json::to_value(vm).context("serialize vm status for inventory")
}
...
async fn allocate_guest_ip_cidr(inner: &Inner) -> Result<String, ApiError> {
    let cidr = inner.defaults.network.guest_cidr.trim();
    let (network_ip_raw, prefix_raw) = cidr
        .split_once('/')
        .ok_or_else(|| ApiError::internal("vm_defaults.network.guest_cidr must be CIDR"))?;
    let network_ip: Ipv4Addr = network_ip_raw
        .parse()
        .map_err(|_| ApiError::internal("vm_defaults.network.guest_cidr must be IPv4 CIDR"))?;
    let prefix: u8 = prefix_raw
        .parse()
        .map_err(|_| ApiError::internal("vm_defaults.network.guest_cidr has invalid prefix"))?;
    if prefix > 30 {
        return Err(ApiError::internal(
            "vm_defaults.network.guest_cidr prefix must be <= 30",
        ));
    }
    ...
    let network_u32 = u32::from(network_ip) & mask;
    let host_capacity = (1_u128 << (32 - prefix)) as u32;

    let used = {
        let states = inner.states.read().await;
        states
            .values()
            .filter_map(|vm| vm.details.as_ref().and_then(|d| d.guest_ip.clone()))
            .collect::<std::collections::BTreeSet<String>>()
    };

    // Skip .0 (network), .1 (gateway), and last broadcast.
    let gateway = inner.defaults.network.gateway.trim().to_string();
    for host_offset in 2..host_capacity.saturating_sub(1) {
        let candidate = Ipv4Addr::from(network_u32.saturating_add(host_offset)).to_string();
        if candidate == gateway || used.contains(&candidate) {
            continue;
        }
        return Ok(format!("{candidate}/{prefix}"));
    }

    Err(ApiError::conflict("guest IP pool exhausted"))
}
...
fn create_recording_disk(path: &Path) -> Result<()> {
    ...
    let options = fatfs::FormatVolumeOptions::new().volume_label(*b"INTARREC   ");
...
mod tests {
    ...
    fn create_scenario_vm_request_accepts_runtime() {
        let req: CreateScenarioVmRequest = serde_json::from_value(json!({
            ...
            "runtime": {
                "stargate_target_public_key_openssh": "ssh-ed25519 AAAATEST stargate-target",
                "network": {
        ...
        assert_eq!(
            req.runtime.stargate_target_public_key_openssh,
            "ssh-ed25519 AAAATEST stargate-target"
        );
        assert_eq!(
            req.runtime
                .network
