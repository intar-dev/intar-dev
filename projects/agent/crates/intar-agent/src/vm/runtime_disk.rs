...
const RUNTIME_DISK_LABEL: [u8; 11] = *b"INTARRUN   ";

pub struct RuntimeDiskInput<'a> {
    pub path: &'a Path,
    pub stargate_target_public_key_openssh: &'a str,
...
pub fn write_runtime_disk(input: &RuntimeDiskInput<'_>) -> Result<()> {
    ...
    write_file(
        ...
        "runtime.env",
        &render_runtime_env(
            input.stargate_target_public_key_openssh,
            input.kino_vsock_cid,
            input.kino_vsock_port,
            input.hostname,
            input.network,
        ),
    )?;
...
fn render_runtime_env(
    stargate_target_public_key_openssh: &str,
    kino_vsock_cid: u32,
    kino_vsock_port: u32,
    hostname: &str,
    ...
    format!(
        concat!(
            "INTAR_STARGATE_TARGET_PUBLIC_KEY_OPENSSH={}\n",
            "KINO_VSOCK_CID={}\n",
            "KINO_VSOCK_PORT={}\n",
            "INTAR_VM_HOSTNAME={}\n",
            "INTAR_GUEST_IP_CIDR={}\n",
        ...
        shell_single_quoted(stargate_target_public_key_openssh),
        shell_single_quoted(&kino_vsock_cid.to_string()),
        shell_single_quoted(&kino_vsock_port.to_string()),
        shell_single_quoted(hostname),
        shell_single_quoted(&network.guest_ip_cidr),
        shell_single_quoted(&network.gateway),
        shell_single_quoted(&network.dns.join(" ")),
    )
}
...
mod tests {
    ...
    fn runtime_disk_contains_runtime_env_file() {
        ...
        assert!(runtime_env.contains(
            "INTAR_STARGATE_TARGET_PUBLIC_KEY_OPENSSH='ssh-ed25519 AAAATEST stargate-target'"
        ));
