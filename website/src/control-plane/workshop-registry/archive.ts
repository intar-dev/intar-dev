  ...
  return {
    ...
    workspace: {
      ...
      vms: sourceVms.map((vm) => ({
        ...
        cpuMillis: positiveInteger(vm.vcpu_millis, "workspace VM vcpu_millis"),
...
function validateCompiledManifest(
  ...
  if (manifest.format_version !== 1) {
    throw invalid("manifest must use format_version 1");
  }
  ...
  for (const vm of vms) {
    string(vm.image, "workspace VM image");
    positiveInteger(vm.vcpu_millis, "workspace VM vcpu_millis");
