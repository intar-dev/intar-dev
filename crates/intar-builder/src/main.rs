...
struct RunOnceCommand {
    ...
    bundle: String,
    /// Ignore reusable layered base and checkpoint state for this build.
    #[arg(long)]
    no_cache: bool,
}
...
async fn run(args: RunCommand) -> Result<()> {
    ...
    for worker_id in 0..PUBLICATION_WORKERS {
        let worker_cfg = cfg.clone();
        let worker_report_tx = report_tx.clone();
        let worker_desired_ready = desired_ready_rx.clone();
        tokio::spawn(async move {
...
async fn run_once(args: RunOnceCommand) -> Result<()> {
    ...
    if args.no_cache {
        cfg.qemu.layered.use_cache = false;
    }
    ...
    info!(
        ...
        buildctl_binary = %build_config.layered.buildctl_binary.display(),
        umoci_binary = %build_config.layered.umoci_binary.display(),
        "builder run-once starting OCI-backed image build"
    );
