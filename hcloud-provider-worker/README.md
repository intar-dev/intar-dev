...
The Worker owns one `HcloudConnectionDO` per organization provider connection.
...
Deploy this Worker as `intar-hcloud-provider`, then add this non-inheritable
...
```ts
import type { HcloudProviderService } from "@intar/hcloud-provider-worker";
import type {
  ConnectProjectRequest,
  ProviderRpcResult,
} from "@intar/hcloud-provider-worker/contracts";

interface WorkerEnv {
  HCLOUD_PROVIDER_SERVICE: Service<HcloudProviderService>;
}
```
...
`PROVIDER_CREDENTIAL_KEK_V1` is a base64-encoded random 32-byte key. It is a
...
`CLOUDFLARE_HCLOUD_PROVIDER_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and
`PROVIDER_CREDENTIAL_KEK_V1`. The Cloudflare token is dedicated to deploying
